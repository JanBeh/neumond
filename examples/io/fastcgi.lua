...
local function connection_handler(conn)
...
local function connection_handler_closable(...)
  fiber.handle(
    close_handlers,
    connection_handler,
    ...
  ...
  function()
    fiber.handle(
      ...
      function()
        ...
        local listener = assert(eio.locallisten("fcgi.sock"))
        while true do
          local conn = listener:accept()
          fiber.spawn(function()
            local conn <close> = conn
            local success, errmsg = effect.xpcall(
              connection_handler_closable, debug.traceback, conn
            )
            if not success then
