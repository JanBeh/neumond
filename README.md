...
Module for algebraic effect handling implemented in pure Lua.
  ...
  * **`effect.discontinue(resume)`** releases a continuation (`resume`) passed
    ...
    is only applicable for continuations generated by `handle_once` but must
    not be used for continuations generated by `handle` as continuations
    generated by the `handle` function are automatically discontinued when an
    effect handler returns (or throws an error) without resuming.
